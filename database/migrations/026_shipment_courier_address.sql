-- Migration 026: structured courier address on order_shipments (stage 2G)
--
-- Stage 2G adds the structured Nova Post courier destination so the
-- provider payload (recipient.settlementId + recipient.addressParts
-- {street, building, flat}) can be built without parsing the free-text
-- `address` display string:
--
--   order_shipments.street_name text
--   order_shipments.building    text
--   order_shipments.flat        text
--
-- Column roles stay unchanged:
--   * city_ref      — Nova Post settlementId (numeric text, integer);
--   * warehouse_ref — Nova Post divisionId (numeric text, integer);
--   * address       — display/compat courier string (still required by
--                     chk_order_shipments_destination for courier rows).
--
-- Scope guard (stage 2G, explicit GO 2026-08-28):
--   * NO new integer city/division columns (city_ref/warehouse_ref keep
--     their roles);
--   * NO orders changes — total_amount / shipping_total / the LiqPay money
--     model are untouched (delivery is paid by the recipient, payerType is
--     fixed server-side and never enters order totals);
--   * NO RLS changes (019 default-deny stays);
--   * admin_replace_shipment_plan (021) is re-created with the ONLY
--     minimal extension: optional street_name/building/flat accepted for
--     courier shipments, rejected for warehouse ones. All existing
--     freeze/validation guarantees (planned-only, no TTN, destination XOR,
--     numeric city_ref/warehouse_ref, COD/allocation triggers) are kept.
--
-- Text limits mirror the provider schema caps visible in the live research
-- (flat max 10; building/street capped conservatively at 100).
--
-- Idempotency: ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE function +
-- guarded grants. Rerunnable.

begin;

-- ==================== STRUCTURED COURIER ADDRESS ====================

alter table order_shipments add column if not exists street_name text;
alter table order_shipments add column if not exists building text;
alter table order_shipments add column if not exists flat text;

-- ==================== ADMIN REPLACE SHIPMENT PLAN ====================
-- Re-created from migration 021 with the minimal stage 2G extension:
-- optional courier address parts (street_name / building / flat) in the
-- plan JSON; warehouse shipments must not carry them.
