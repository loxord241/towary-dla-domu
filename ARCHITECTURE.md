...
Три товарных направления (укр. контент):
...
- **Линолеум** — нарезной товар, sku `ln-*`, витрина `/linoleum` (ISR 60 + dynamic twin), остатки из 1С 7.7 (первый реальный экспорт 2026-09-18): 1 карточка = дизайн, ширины рулонов = product_variants, цена = грн/пог.м, сток = целые погонные метры; тех-характеристики сайта при синке сохраняются (L12).
...
### Линолеум (1С, вручную от владельца)
- Запуск 2026-09-18 (вертикаль по плану владельца): sku `ln-*` (третий домен
  в `domains.ts`), staging `linoleum_stock` (052), ingest
  `/api/ingest/1c-linoleum` (bearer `LINOLEUM_INGEST_SECRET`, CSV
  `code;name;width_m;price_sqm;qty_m`, ширины {1.5,2,2.5,3,3.5,4}),
  `scripts/1c-export/xls-linoleum-to-ingest-csv.py` (чистка имён дизайна,
  секции ширин, floor дробных метров), `scripts/linoleum-import.ts
  --plan|--run|--publish` (publish — единственный писатель is_active,
  фото-гейт). Фото: `scripts/linoleum-photos.ts --run` (карта
  `data/linoleum-photos.json`, imageUrl/localPath).
- Модель: 1 карточка = дизайн, ширины = `product_variants` (цена = грн/пог.м,
  сток = целые метры), `products.price` = MIN пог.м, specs = «Ціна за м²»
  (MIN) + «Ширина»×N (фильтр хаба jsonb contains) + тех-записи сайта
  (импортёр их сохраняет, L12). products.stock_quantity = сумма вариантов
  на момент импорта (после продаж реальный сток в вариантах).
- Checkout: панель метража на PDP (вариант-чипы ширин + целые метры 1–99),
  калькулятор геометрический (ориентация + стыковка), самовывоз — Мазепи 89А
  (fail-closed серверная проверка точка↔корзина). Пагинация витрин —
  документ-навигация (гонка segment-cache, см. PaginationNav).
