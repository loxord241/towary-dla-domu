...
test('JUNCTION-FILTER: both queries embed pc and filter by subtree ids', () => {
  ...
  // The pc embed joins only via the categoryId ternary guards.
  assert.match(s, /categoryId\s*\?\s*\n?\s*ELIGIBLE_COUNT_SELECT \+ ', pc:product_categories!inner\(id\)'/);
  assert.match(s, /categoryId\s*\?[\s\S]{0,120}?PRODUCT_SELECT \+ ', pc:product_categories!inner\(category_id\)'/);
});
