...
test('linoleum-pdp: хлібні крихти не дублюють назву доменної категорії на PDP (P2.2)', () => {
  ...
  // Шпалери: без підкатегорій (тільки корінь 'shpaleri')
  assert.deepEqual(
    renderBreadcrumbs(
      false,
      true,
    ...
    ['Головна', 'Шпалери', 'Шпалери 10м'],
    'шпалери: «Головна / Шпалери / <товар>» без другого «Шпалери» при відсутності підкатегорій'
  );

  // Лінолеум з кількома рівнями підкатегорій: зберігає всі підкатегорії
  assert.deepEqual(
    renderBreadcrumbs(
      true,
      false,
      [
        { slug: 'linoleum', name: 'Лінолеум' },
        { slug: 'napivkomertsiinyi', name: 'Напівкомерційний' },
        { slug: 'tarkett', name: 'Tarkett' },
      ],
      'Таркетт'
    ),
    ['Головна', 'Лінолеум', 'Напівкомерційний', 'Tarkett', 'Таркетт'],
    'лінолеум: багаторівневий ланцюжок зберігає всі вкладені підкатегорії'
  );

  // Порожній trail: рендериться кореневий домен + товар
  assert.deepEqual(
    renderBreadcrumbs(true, false, [], 'Таркетт'),
    ['Головна', 'Лінолеум', 'Таркетт'],
    'лінолеум: порожній trail'
  );
  assert.deepEqual(
    renderBreadcrumbs(false, true, [], 'Шпалери 10м'),
    ['Головна', 'Шпалери', 'Шпалери 10м'],
    'шпалери: порожній trail'
  );
