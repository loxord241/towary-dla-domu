 ...
 * next.config 301-redirects ONLY allowlisted pairs (never the 6 price-diff
...
test('DU: next.config 301-redirects exactly the JSON pairs, nothing else', () => {
  ...
  // JSON is the config source: same 97 slugs, same slugs as the TS allowlist.
  assert.equal(duRedirectsJson.redirect.length, 97);
  assert.deepEqual(
