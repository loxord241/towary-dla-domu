...
## Гигиена рабочего дерева и хендовер

- Начал сессию — сначала `git status` + `git diff --stat`: грязное дерево может
  быть незавершённой работой владельца или другого агента. Не откатывай и не
  стирай чужие незакоммиченные правки; непонятное — спроси.
- Заканчивая, оставь дерево описываемым: либо коммит по задаче, либо перечень
  незакоммиченного в финальном отчёте с причиной («ждёт GO», «throwaway»).
  Хендовер = следующий агент может продолжить: что сделано, что проверено,
  что заблокировано и на чём.
- Throwaway-файлы (`scripts/tmp-*.ts`): шапка с целью и условием удаления;
  условие наступило — удали, не копи.
- Коммит без пуша — легален: агенты ЭТОГО же чекута видят и рабочее дерево,
  и локальные коммиты сразу; агенты других клонов/машин (WSL-копия, CI,
  Vercel) — только запушенное. Держи пуш-отставание осознанным и упоминай
  его в отчёте.

### Статусы документов

- **Живые** (актуализируются при изменении фактов): `AGENTS.md`,
  `ARCHITECTURE.md`, `PROJECT_CONTEXT.md` (журнал: записи исторические,
  дополнения живые), `docs/windows-sync.md`, `docs/wsl-sync.md`,
  `docs/windows-migration.md`. Противоречие коду — чинить документ.
- **Снапшоты** (датированы, НЕ актуализируются): `docs/*audit*.md`,
  `docs/*-research*.md`, `docs/superpowers/specs/*.md` — фиксация состояния
  на дату; текущую истину сверять с кодом.
- Новый документ без явного статуса = снапшот с датой в имени.
