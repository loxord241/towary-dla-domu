/**
 * GET /api/cron/wallpaper-freshness — daily freshness check of the 1С
 * wallpaper stock channel (wallpapers import, plan Task 9; spec §6:
 * docs/superpowers/specs/2026-09-10-wallpapers-import-design.md).
 *
 * The owner's PC exports the 1С stock file DAILY (14:00 local) into the
 * `wallpaper_stock` staging table (via /api/ingest/1c-wallpaper). This cron
 * closes the "channel silently died" gap: it reads max(export_date) from
 * staging and alerts the owner's Telegram channel when
 *   - staging has 0 rows (no file has EVER arrived), or
 *   - the freshest export_date is older than 26 h (a missed daily export,
 *     with headroom for the 14:00 schedule).
 *
 * Security model (app/api/cron/reconciliation/route.ts):
...
/** Spec §6: a daily channel; >26 h without a file means it is down. */
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    // append-only, migration 041; RLS on, service role only).
    ...
    console.error(
      'cron wallpaper freshness failed:',
  ...
  let notified = false;
  if (stale) {
    const text = `Wallpaper sync: файл из 1С не поступал более 26 ч (последний: ${lastExportDate ?? '—'})`;
    try {
      const result = await sendTelegramText(text);
      notified = result.sent;
    } catch (error) {
      console.error(
        'cron wallpaper freshness telegram failed:',
